...
beforeAll(async () => {
  ...
  await new Promise(/** @param {(value: unknown) => void} resolve */ (resolve) => {
    server.listen(TEST_PORT, () => {
      console.log(`Auth lifecycle test server started on port ${TEST_PORT}`);
      resolve(true);
    });
  });
...
afterAll(async () => {
  await new Promise(/** @param {(value: unknown) => void} resolve */ (resolve) => {
    server.close(() => {
      console.log('Auth lifecycle test server closed');
      resolve(true);
    });
  });
}, 15000); // Increased timeout to 15 seconds
...
describe("Authentication Lifecycle Tests - Development", () => {
  ...
  test("4. Token Refresh - POST /api/auth/refresh", async () => {
  ...
  test("5. User Logout - POST /api/auth/logout", async () => {
    ...
    // Token should still work (we don't invalidate access tokens)
    // This is normal JWT behavior - we'd need something like Redis to blacklist tokens
    console.log(`Verify after logout status: ${verifyResponse.status}`);
  });
  
  test("6. Account Cleanup - DELETE /api/auth/users/:id", async () => {
