...
// Development test suite for the API message functionality
test.describe('Development - API Message Connection Tests', () => {
  ...
  test('should display API section with correct button state', async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'dev-api-initial-state.png') });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'dev-api-section.png') });
  });
  ...
  test('should show error message when API connection fails in development', async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'dev-api-connection-error.png') });
  });
  ...
  test('should show success message when API connection succeeds in development', async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'dev-api-connection-success.png') });
  });

  test('should test both API and SQLite connections in development', async ({ page }) => {
    ...
    await page.route('/api/db-status', route => {
      route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          message: 'SQLite database is connected and operational in DEVELOPMENT mode',
          status: 'healthy',
          timestamp: new Date().toISOString()
        })
      });
    ...
    await expect(dbMessage).toContainText('SQLite database is connected');
    await expect(dbButton).toHaveClass(/bg-green-600/);
    
    // Take a screenshot of both successful tests
    await page.screenshot({ path: path.join(screenshotDir, 'dev-both-connections-success.png') });
  });
});
