...
// Test suite for demonstrating successful connection states
test.describe('Development - All Connection Success States', () => {
  ...
  test('should display successful connections for both API and SQLite', async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'dev-all-buttons-initial.png') });
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'dev-api-success-only.png') });
    ...
    await page.screenshot({ 
      path: path.join(screenshotDir, 'dev-all-connections-success.png'),
      fullPage: true 
    });
  });
});
